...
from core.constants import KEYWORD_DOMAIN_EXTRACTION_LLM, WORKLET_GENERATOR_LLM, REFERENCE_KEYWORD_LLM, REFERENCE_SORT_LLM
...
async def process_input(state: AgentState) -> AgentState:
    ...
    if state.files and len(state.files) > 0:
        await sio.emit('status_update', {'message': 'Uploading and processing files...'})
...
async def extract_keywords_domains(state: AgentState) -> AgentState:
    ...
    await sio.emit(f"{state.thread_id}/status_update", {"message": "Extracting keywords and domains..."})
...
async def generate_worklets(state: AgentState) -> AgentState:
    ...
    await sio.emit(f"{state.thread_id}/status_update", {"message": "Generating worklets..."})
...
async def web_search(state: AgentState) -> AgentState:
    ...
    await sio.emit(f"{state.thread_id}/status_update", {"message": "Web search invoked..."})
    s = time.time()
    ...
    await sio.emit(f"{state.thread_id}/status_update", {"message": "Web search completed."})
    return state
...
async def references(state: AgentState) -> AgentState:
    ...
    for worklet in state.generation_output.worklets:
        await sio.emit(f"{state.thread_id}/status_update", {"message": f"Generating references for worklet: {worklet.title}..."})
        prompt = keyword_prompt(worklet.title or worklet.problem_statement)
        try:
            result: ReferenceKeywordResult = await invoke_llm(gpu_model=REFERENCE_KEYWORD_LLM.model, contents=prompt, response_schema=ReferenceKeywordResult, port=REFERENCE_KEYWORD_LLM.port)
            keywords = result or ReferenceKeywordResult(google_scholar_keyword=worklet.title, github_keyword=worklet.title)
            with open(f"debug/reference_keyword_{worklet.title}.txt", "w", encoding="utf-8") as f:
                f.write(str(result.model_dump()))
        except Exception as e:
            print(f"Error extracting reference keyword for worklet '{worklet.title}': {e}")
            keywords = ReferenceKeywordResult(google_scholar_keyword=worklet.title, github_keyword=worklet.title)
...
async def sort_references(state: AgentState) -> AgentState:
    for worklet in state.worklets:
        await sio.emit(f"{state.thread_id}/status_update", {"message": f"Sorting references for worklet: {worklet.title}..."})
...
async def generate_files(state: AgentState) -> AgentState:
    ...
    db.threads.update_one({"thread_id": state.thread_id}, {"$set": {"worklets": [w.model_dump() for w in state.worklets]}})

    s = time.time()
    for idx, worklet in enumerate(state.worklets):
        await sio.emit(f"{state.thread_id}/status_update", {"message": f"Generating files for worklet: {worklet.title}..."})
